...
#'
#' @param ll List. List to be checked.
#' @param nms Character Vector. Required element names in the list.
#'
...
check_list_str <- function(ll, nms) {
  if (!is.null(ll)) {
    ...
    if (!is.list(ll) || length(ll)!=length(nms)) {
      stop(paste0("`", nm ,"` has to be a list with ",
                  length(nms)," element(s)!"))
    }
    if (length(nms)==1 && nms == "") {
      names(ll) <- NULL
    } else {
      if (!all(nms %in% names(ll))) {
        stop(paste0("Names of elements in `", nm ,"` have to be ",
                    paste0(nms, collapse=", "), "!"))
      }
      ll <- ll[nms]
    }
    if (length(unique(lengths(ll))) > 1L) {
      stop(paste0("All elements in `", nm,
